...
export class LoadMoreItemsResponse extends TweeterResponse{
    ...
    public _items: [User[], boolean]
    

    public constructor(success: boolean, items: [User[], boolean], message:string){
    ...
    static fromJson(json: LoadMoreItemsResponse): LoadMoreItemsResponse {
        ...
        let deserializedUserArray:User[] = [];
        ...
        // You can access properties of jsonValue using dot notation
          if(jsonValue == true){
            ...
            for(let jsonObject in jsonValue){
              ...
              const deserializedUser = User.fromJson(JSON.stringify(specificUser)); 
              console.log("Deserialized User: "+ deserializedUser?.alias)
        ...
        const itemsArray: [User[], boolean] = [deserializedUserArray, deserializeHasMorePages]
